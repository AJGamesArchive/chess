...
export function checkHighlight(checkState: CheckDetails, chessboard: any[][], highlightColor: string, darkSquareColor: string, highlight: boolean): any[][] {
  if (highlight) {
    ...
    for (const square of checkState.puttingInCheckSquare) {
      ...
      pieceSquare.color = highlightColor;
    };
  } else {
    let kingsSquare: any = chessboard[checkState.kingInCheckSquare.row][checkState.kingInCheckSquare.col];
